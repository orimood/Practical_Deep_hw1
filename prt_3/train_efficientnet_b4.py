"""
EfficientNet-B4 Transfer Learning - Fish Classification
...
print(f"\n{'='*70}")
print(f"EFFICIENTNET-B4 - FISH CLASSIFICATION")
print(f"{'='*70}")
...
def plot_confusion_matrix(cm: np.ndarray, class_names: List[str], save_path: Path):
    ...
    plt.title("Confusion Matrix – EfficientNet-B4 OOF")
    plt.tight_layout()
    save_path.parent.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    plots_dir = project_root / "plots" / "EfficientNet-B4"
    results_dir = project_root / "results" / "EfficientNet-B4"
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(all_indices, all_labels), start=1):
        ...
        model = load_efficientnet_b4(num_classes).to(device)
        ...
        save_per_sample_predictions(val_preds, val_labels, val_probs, val_image_paths, base_dataset.class_names, predictions_file, fold_idx, "EfficientNet-B4")
