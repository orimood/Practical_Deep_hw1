...
\begin{figure}[h]
    \centering
    [FIGURE PLACEHOLDER: Class distribution plot]
...
\begin{figure}[h]
    \centering
    [FIGURE PLACEHOLDER: Fish samples grid - 9 species]
...
\begin{figure}[h]
    \centering
    [FIGURE PLACEHOLDER: Distinct species comparison]
...
\begin{figure}[h]
    \centering
    [FIGURE PLACEHOLDER: Red Mullet vs Striped Red Mullet comparison]
...
\begin{figure}[h]
    \centering
    [FIGURE PLACEHOLDER: Gilt-Head Bream vs Red Sea Bream comparison]
...
\begin{enumerate}
    \item \textbf{Support Vector Machine (RBF kernel)}: $C=10$, $\gamma=$`scale'
...
\begin{enumerate}
    ...
    \item \textbf{Fine-tuning may be unnecessary}: For this dataset size ($\approx$9,000 images per class), the pretrained features alone are sufficient
...
\begin{itemize}
    \item \textbf{SVM (Linear)}: 1.28 seconds training time — 2000$\times$ faster than fine-tuning
    \item \textbf{SVM (RBF)}: 2.70 seconds — Still 1000$\times$ faster
