"""
MobileNet-V3 Large Transfer Learning for Fish Species Classification
Assignment 1, Question 3 - Model 3/4

Key Features:
- Pretrained MobileNet-V3 Large from torchvision
- Fastest and smallest model (~5.4M parameters)
...
def get_transforms():
    """Get image transforms for MobileNet-V3 (224x224 input)"""
...
def main():
    ...
    wandb.init(
        ...
        config=config,
        name="mobilenet-v3-large-transfer-learning"
    )
    config = wandb.config
    ...
    # Create datasets
    print("\n" + "="*70)
    print("MOBILENET-V3 LARGE TRANSFER LEARNING - Question 3, Model 3/4")
    print("="*70)
    ...
    # Load pretrained MobileNet-V3 Large and modify final layer
    print("\n" + "="*70)
    ...
    # MobileNetV3 has a 'classifier' attribute with multiple layers
    num_features = model.classifier[3].in_features
    model.classifier[3] = nn.Linear(num_features, len(train_dataset.class_names))
    ...
    best_val_acc = 0.0
    best_model_path = project_root / "models" / "best_mobilenet_v3_large.pth"
    ...
    print("="*70)
    print(f"Model Name: MobileNet-V3 Large")
    print(f"# Parameters: {total_params:,} (~5.4M)")
