"""
MobileNetV3-Large Transfer Learning - Fish Classification
...
NUM_FOLDS = 5
NUM_EPOCHS = 18
BATCH_SIZE = 64  # Larger batch for mobile model
LEARNING_RATE = 1e-4
NUM_WORKERS = 0
IMAGE_SIZE = 224
PATIENCE = 4
...
print(f"\n{'='*70}")
print(f"MOBILENETV3-LARGE - FISH CLASSIFICATION")
print(f"{'='*70}")
...
def plot_confusion_matrix(cm: np.ndarray, class_names: List[str], save_path: Path):
    ...
    plt.title("Confusion Matrix – MobileNetV3-Large OOF")
    plt.tight_layout()
    save_path.parent.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    plots_dir = project_root / "plots" / "MobileNetV3-Large"
    results_dir = project_root / "results" / "MobileNetV3-Large"
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(all_indices, all_labels), start=1):
        ...
        model = load_mobilenetv3(num_classes).to(device)
        ...
        save_per_sample_predictions(val_preds, val_labels, val_probs, val_image_paths, base_dataset.class_names, predictions_file, fold_idx, "MobileNetV3")
    ...
    save_per_sample_predictions(oof_preds, all_labels, oof_probs, all_oof_image_paths, base_dataset.class_names, oof_predictions_file, 0, "MobileNetV3")
